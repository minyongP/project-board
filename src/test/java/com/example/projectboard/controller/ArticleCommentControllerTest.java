...
class ArticleCommentControllerTest {
    ...
    @WithUserDetails(value = "myTest", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    @DisplayName("[view][post] 댓글 등록 - 정상 호출")
    @Test
    void givenArticleCommnetInfo_whenRequesting_thenSavesNewArticleComment() throws Exception {
        //given
        long articleId = 1L;
        ...
        mvc.perform(post("/comments/new")
                        ...
                        .content(formDataEncoder.encode(request)).with(csrf())
                )
    ...
    void givenArticleCommentIdToDelete_whenRequesting_thenDeletesArticleComment() throws Exception {
        ...
        mvc.perform(post("/comments/" + articleCommentId + "/delete")
                .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                .content(formDataEncoder.encode(Map.of("articleId", articleId))).with(csrf())
        )
                .andExpect(status().is3xxRedirection())
                .andExpect(view().name("redirect:/articles/" + articleId))
                .andExpect(redirectedUrl("/articles/" + articleId));
    ...
    @Test
    void givenArticleCommentsInfoWithParentCommentId_whenRequesting_thenSavesNewChildComment() throws Exception {
        // Given
        long articleId = 1L;
